...
class TestGuestAccountEdgeCases:
    def test_guest_email_is_unique(self, client):
        ...
        email1 = client.get("/api/auth/me", headers=auth_header(resp1.json()["access_token"])).json()["email"]
        email2 = client.get("/api/auth/me", headers=auth_header(resp2.json()["access_token"])).json()["email"]
    ...
    def test_upgrade_can_login_with_new_credentials(self, client):
        ...
        client.post("/api/auth/upgrade", json={
...
class TestGDPRExport:
    ...
    def test_export_with_resumes(self, client):
        ...
        # Create resumes
        client.post("/api/resumes", json={"name": "CV 1", "json_content": {"personal": {"name": "John"}}}, headers=headers)
